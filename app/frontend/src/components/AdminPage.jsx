...
export default function AdminPage({ loggedInUser, setLoggedInUser, onLogout, ctfActive, ctfSecondsLeft }) {
  ...
  // -------------------------
  useEffect(() => {
    const open = !!deleteTarget || !!statusTarget;
    if (!open) return;
  ...
  // -------------------------
  const bg = useMemo(() => createPortal(<AdminBg />, document.body), []);
  if (!isAdmin) {
    return (
      <>
        {bg}
...
function AdminBg() {
  return (
    <div className="fixed inset-0 z-0 pointer-events-none" style={{ zIndex: -1 }} aria-hidden="true">
      <div
